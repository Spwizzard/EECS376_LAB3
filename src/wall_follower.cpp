...
const double YAW_RATE = 0.5; //0.1; // rad/sec yaw rate command
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    ...
    if (g_index_front < 0) {
        ...
        g_index_90deg_left = (int) ((2 * g_angle_max ) / g_angle_increment);
		ROS_INFO("Left most ping index %d: ",g_index_90deg_left );
	...
	if(isnan(g_radius_left)) {
		g_radius_left = g_range_max;
	}
	if(isnan(g_clearance_tan_test)) {
		g_clearance_tan_test = .7
;
	}
...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
        ...
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left)) {
            ...
            double dtheta = -M_PI/2 + g_angle_max + ((double) (g_index_min_dist_ping - g_index_90deg_left)) * g_angle_increment - 1.0;
